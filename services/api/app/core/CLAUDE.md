...
- **Parametre adı yüzde kodlu gelebilir ve derinliği sınırsızdır.** `X-Amz-%53ignature` sunucu için hâlâ `X-Amz-Signature`, `%2553` ise `%53`'e çözülür. Sabit sayıda çözme turu bunu kapatmaz; kalıp her karakterin yerine `%(?:25)*XX` kabul eder ve maskeleme **ham biçimde** uygulanır. Hızlı yolun güvenliği iki dallı bir argümandır ve ikisi de testlidir: adın harfi harfine yazılmış hâli bir işaretçi içerir, başka türlü yazılmış hâli ise `%` gerektirir.
