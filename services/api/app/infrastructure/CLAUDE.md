...
| `render/ffmpeg.py` | `FFmpegRenderAdapter` — `RenderPort`'un FFmpeg uygulaması: filtre grafiği, concat demuxer, altyazı yakma, **seslendirme birleştirme + altlığa miksaj ve ducking** (W19), timeout ve kısmi çıktı temizliği; `ffmpeg` sözcüğü bu çizginin altında kalır. Miksaj `filter_complex` içinde, çünkü sidechain kompresörünün iki sinyali de aynı grafta olmalı; seslendirmesiz timeline eski yolu (`0:a`, boş graf) birebir korur |
| `render/fake.py` | `FakeRenderAdapter` — yer tutucu dosya yazan render fake'i (birim testler FFmpeg olmadan render *servisini* sınar); üretimde reddedilir. Kabiliyet kümesi gerçek adapter'la birebir aynı kalır — ayrışırsa buradaki her test hiçbir şeyin testi olur |
