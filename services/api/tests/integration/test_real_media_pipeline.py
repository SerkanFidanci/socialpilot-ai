    ...
    try:
        async with engine.begin() as connection:
            await connection.execute(
                text(
                    "TRUNCATE media_scene_understandings, transcript_segments, transcripts, "
...
def test_phase_1_exit_criterion_with_three_real_videos(tmp_path: Path) -> None:
    ...
    assert make_video(
        vertical, size="240x426", codec="libx264", container_args=[], audio=False
    ), "cannot encode the vertical fixture"
    assert make_video(
        voiced, size="426x240", codec="libx264", container_args=[], audio=True
