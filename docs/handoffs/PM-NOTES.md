...
**Codex teyit turu DÖNDÜ (2026-08-01): W15 TEMİZ (6/6 — kapandı, dal+worktree silindi). W16 delindi:** 2 kritik + 1 orta — Coptic `Ⲧ` (U+2CA6) confusable tablosunda yok, atanmamış görünmez U+2065 (`Cn`) temizlenmiyor (ikisi de kalıcı `generated` script'e ulaştı), yüzde-kodlu parametre adları (`X-Amz-%53ignature`) redaksiyon fast-path'ini atlatıyor. **Düzeltme turu 2 sıcak oturumda** — talimat W16 dosyasının "Düzeltme turu 2" bölümünde (sınıf-kapatma şartı: confusables üretilmiş tablodan/mixed-script kuralı, `Cn`+`Co` kategori temizliği, yüzde-çözülmüş kopyada arama). Tetikleme prompt'u (W16 oturumu açıksa ona, değilse yeni oturuma):
```
docs/handoffs/W16-verification-followups-3.md dosyasındaki "Düzeltme turu 2" bölümünü oku ve uygula. Aynı dalda (fix/verification-followups-3) çalış; önce git merge main ile dalını güncelle. Worktree kökünden ve COMPOSE_PROJECT_NAME=sp-w16 ile çalıştır. Migration yok. Merge etme, dalda bırak.
```

**Düzeltme dönünce akış:** PM denetler → merge → tam doğrulama (taban **743**) → push → kısa Codex teyidi (yalnız yeni düzeltme yüzeyleri). Temizse → **W17 (kalıp grameri) yaz ve tetikle**, sonra **2D (QC)**. W17 kapsamı (PM kararları verildi): diyakritiksiz Türkçe katlaması **yalnızca** `find_fabrication`/`contains_url`/yasak-terim eşleşmesi için (ONAY: yasak terimler de katlanır — `şeker` yasaksa `seker` de yasak; `_scene_tags`'in sakladığı değere UYGULANMAZ), `T.L.`/`T L` kısaltması, NFKC'nin `⑴⑸`→`(1)(5)` açtığı bitişiklik. 2D kapsamı: Phase 2 planı §3 + `forbidden_matcher` birleştirmesi (`text_normalization.py`'ı import eder) + 2C süre sapması eşikleri. W17 önce (ikisi de `text_normalization.py`'a dokunabilir).
