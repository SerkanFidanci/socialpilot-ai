...
| [W08](handoffs/W08-provider-benchmark-harness.md) | **Golden set benchmark koşum takımı** — gerçek AI sağlayıcısı bağlamanın ön koşulu | **tamamlandı** · merge bekliyor · **PM'e:** `provider_usage` tablosu kodda yok (ADR-007 metni düzeltilmeli); harness `app/benchmark/`'e kondu (domain-dışı) | `slice/1h-provider-benchmark` | Opus 5 / high | — |
