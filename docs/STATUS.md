...
| | |
|---|---|
| `main` | W01→W19 merge edildi (2A–2E birinci yarı). **Düzeltme turu 4 dalda tamamlandı** — QC birleştirmesi sıra-bağımsız (en kötü kazanır), ondalık kesir sözcükleri kapalı; `fix/verification-followups-4`, **1237 pytest**, merge bekliyor. W19 için Codex turu bekliyor |
| Alembic head | `0016_content_projects` (tek head; zincir 0001→0016, up/down/up doğrulandı) |
| Backend doğrulama | `main`: **1204 pytest**; **`fix/verification-followups-4` dalında 1237** (gerçek PostgreSQL + MinIO + FFmpeg) · lint + format + mypy strict temiz · py313 / mypy 2.3 / ruff 0.16 / pytest 9.1 |
