...
| Backend doğrulama | **1375 pytest** (gerçek PostgreSQL + MinIO + FFmpeg; merge sonrası PM koşusu) · lint + format + mypy strict temiz · py313 / mypy 2.3 / ruff 0.16 |
...
| [W22](handoffs/W22-content-planner.md) | **Phase 2G — içerik planlayıcı** (§13, Phase 2'nin son dilimi): `content_obligations`, on önceliğin yedisi (4 ve 10 alan olarak açık), sessiz saatler + tenant timezone, karma **ölçülür yargılanmaz**, obligation→proje idempotent ve kredi rezerve ederek, `APPROVED → SCHEDULED` | **şimdi** | `slice/2g-content-planner` | Opus 5 / high | **SENDE** (`0019`) |
