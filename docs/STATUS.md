...
| [W07](handoffs/W07-single-server-resilience.md) | **Tek sunucu dayanıklılığı** — kaynak limitleri + sunucu dışına yedek + geri yükleme provası | **uygulandı · Docker doğrulandı (295 test + backup/restore loop) · PM: ADR no ver + merge · bağımsız doğrulama bekliyor** | `slice/0l-single-server-resilience` | Opus 4.8 / medium | — |
