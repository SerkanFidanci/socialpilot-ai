...
| [W08](handoffs/W08-provider-benchmark-harness.md) | **Golden set benchmark koşum takımı** — gerçek AI sağlayıcısı bağlamanın ön koşulu | **tamamlandı** · doğrulandı (282 pytest + strict) · **PM merge edecek** (W07 ile `Makefile` merge-çakışması; ADR numaralandırma PM'de). Bulgular: `provider_usage` tablosu kodda yok → kabul edildi, **W04 migration slotuna alındı**, `ProviderUsageRecord` şekli bırakıldı; harness `app/benchmark/`'te (domain-dışı) | `slice/1h-provider-benchmark` | Opus 5 / high | — |
